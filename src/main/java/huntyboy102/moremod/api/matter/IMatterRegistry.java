 ...
 * values for Items are stored. The main implementation of this is
 * {@link MatterRegistry}.
 */
public interface IMatterRegistry {
	 ...
	 * @return the amount of matter the item stack contains. Returns 0 if the stack
	 ...
	 * @return the Matter Entry associated with that item. It holds all the
	 *         Handlers.
	 */
	IMatterEntry<?, ?> register(Item item, IMatterEntryHandler<?> handler);

	/**
	 * Acts the same way as {@link #register(Item, IMatterEntryHandler)} but for
	 * ores.
	 *
	 * @param ore     the ore dictionary entry.
	 * @param handler the handler.
	...
	/**
	 * Calculates the matter contained in an item based on it's recipe. This is
	 * where the blacklist comes. When an item in the recipe is blacklisted or does
	 * not contain any matter. The calculation will stop and return 0.
	 *
	...
	/**
	 * This add a whole mod the the blacklist. All items from that mod are treated
	 * as blacklisted.
	 *
