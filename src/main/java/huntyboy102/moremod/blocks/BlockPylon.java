...
import huntyboy102.moremod.api.wrench.IWrenchable;
...
public class BlockPylon extends MOBlockContainer<TileEntityMachineDimensionalPylon> implements IWrenchable {
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		return super.getActualState(state, worldIn, pos).withProperty(CTM,
				Loader.isModLoaded("ctm") && state.getValue(TYPE) == MultiblockType.MAIN);
	}
	...
	public boolean onWrenchHit(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side,
		...
		if (pylon != null && pylon instanceof TileEntityMachineDimensionalPylon) {
			return ((TileEntityMachineDimensionalPylon) pylon).onWrenchHit(stack, player, world, pos, side, hitX, hitY,
					hitZ);
		}
		return false;
	}
	...
	public EnumBlockRenderType getRenderType(IBlockState state) {
		return state.getValue(TYPE) == MultiblockType.DUMMY ? EnumBlockRenderType.INVISIBLE
				: super.getRenderType(state);
	}
