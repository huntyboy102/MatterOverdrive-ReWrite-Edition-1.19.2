...
public class TextureArray {
	...
	public TextureArray(TextureAtlasSprite particle, TextureAtlasSprite northTexture, TextureAtlasSprite southTexture,
			TextureAtlasSprite upTexture, TextureAtlasSprite downTexture, TextureAtlasSprite eastTexture,
			TextureAtlasSprite westTexture) {
	...
	public TextureAtlasSprite[] getTextureArray() {
		return new TextureAtlasSprite[] { getNorthTexture(), getEastTexture(), getSouthTexture(), getWestTexture(),
				getUpTexture(), getDownTexture() };
	}
	...
	public String toString() {
		return new ToStringBuilder(this).append("particle", particle).append("northTexture", northTexture)
				.append("southTexture", southTexture).append("upTexture", upTexture).append("downTexture", downTexture)
				.append("eastTexture", eastTexture).append("westTexture", westTexture).toString();
	}
	...
	public boolean equals(Object o) {
		...
		return new EqualsBuilder().append(getParticleTexture(), that.getParticleTexture())
				.append(getNorthTexture(), that.getNorthTexture()).append(getSouthTexture(), that.getSouthTexture())
				.append(getUpTexture(), that.getUpTexture()).append(getDownTexture(), that.getDownTexture())
				.append(getEastTexture(), that.getEastTexture()).append(getWestTexture(), that.getWestTexture())
				.isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(getParticleTexture()).append(getNorthTexture())
				.append(getSouthTexture()).append(getUpTexture()).append(getDownTexture()).append(getEastTexture())
				.append(getWestTexture()).toHashCode();
	}
}