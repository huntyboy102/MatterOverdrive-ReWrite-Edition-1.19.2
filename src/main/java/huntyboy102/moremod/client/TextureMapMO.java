...
public class TextureMapMO extends TextureMap {
	...
	public void loadTextureAtlas(IResourceManager resourceManager) {
		...
		TextureUtil.allocateTextureImpl(this.getGlTextureId(), 0, stitcher.getCurrentWidth(),
		...
		for (TextureAtlasSprite textureatlassprite1 : stitcher.getStichSlots()) {
			...
			if (textureatlassprite1 == this.missingImage
					|| this.generateMipmaps(resourceManager, textureatlassprite1)) {
				...
				try {
					TextureUtil.uploadTextureMipmap(textureatlassprite1.getFrameTextureData(0),
							textureatlassprite1.getIconWidth(), textureatlassprite1.getIconHeight(),
							textureatlassprite1.getOriginX(), textureatlassprite1.getOriginY(), false, false);
	...
	private boolean generateMipmaps(IResourceManager resourceManager, final TextureAtlasSprite texture) {
		...
		try {
			...
			crashreportcategory.addDetail("Sprite size",
					() -> texture.getIconWidth() + " x " + texture.getIconHeight());
	...
	private ResourceLocation getResourceLocation(TextureAtlasSprite p_184396_1_) {
		...
		return new ResourceLocation(resourcelocation.getNamespace(),
				String.format("%s/%s%s", this.basePath, resourcelocation.getPath(), ".png"));
	}
}