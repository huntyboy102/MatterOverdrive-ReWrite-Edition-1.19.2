...
package huntyboy102.moremod.client.model.baked;
...
import huntyboy102.moremod.client.model.ModelBase;
import huntyboy102.moremod.client.model.ModelUtil;
...
public abstract class BakedModelBase implements IBakedModel {
	...
	public BakedModelBase(IModelState state, VertexFormat format,
			Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter, ModelBase model) {
		this.getter = bakedTextureGetter;
		this.format = format;
		textureArray = new TextureArray(this.getter.apply(model.textures.get("particle")),
				this.getter.apply(model.textures.get("north")), this.getter.apply(model.textures.get("south")),
				this.getter.apply(model.textures.get("up")), this.getter.apply(model.textures.get("down")),
				this.getter.apply(model.textures.get("east")), this.getter.apply(model.textures.get("west")));
	}
