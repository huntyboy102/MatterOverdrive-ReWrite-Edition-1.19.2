	...
	public void onRenderWorldLast(RenderHandler handler, RenderWorldLastEvent event) {
		...
		if (!player.getHeldItem(EnumHand.MAIN_HAND).isEmpty()) {
			...
			if (heldItem.getItem() instanceof IBlockScanner
					&& ((IBlockScanner) heldItem.getItem()).showsGravitationalWaves(heldItem)) {
				holdingPad = true;
			} else {
		...
		if (holdingPad && !heldItem.isEmpty()) {
			...
			Vec3d viewEntityPos = renderViewEntity.getPositionEyes(event.getPartialTicks()).subtract(0,
					renderViewEntity.getEyeHeight(), 0);
			if (lastY == 0) {
				lastY = 64;
			}
			...
			for (int x = 0; x < vewDistance; x++) {
				for (int z = 0; z < vewDistance; z++) {
					...
					float yPos = MatterOverdrive.MO_WORLD.getDimensionalRifts().getValueAt(new Vec3d(
							viewEntityPosRound.x + x - vewDistance / 2, 0, viewEntityPosRound.z + z - vewDistance / 2));
					yPos *= Math.sin((x / (double) vewDistance) * Math.PI)
							* Math.sin((z / (double) vewDistance) * Math.PI);
					points[x][z] = yPos;
				}
			}
			...
			for (int z = 0; z < vewDistance; z++) {
				for (int x = 0; x < vewDistance; x++) {
					if (points[x][z] > 0.01) {
						...
						worldRenderer.pos(xPos, getPointSafe(x, z + 1) * height, zPos + 1).color(r, g, b, 1)
								.endVertex();
						worldRenderer.pos(xPos + 1, getPointSafe(x + 1, z + 1) * height, zPos + 1).color(r, g, b, 1)
								.endVertex();
						worldRenderer.pos(xPos + 1, getPointSafe(x + 1, z) * height, zPos).color(r, g, b, 1)
								.endVertex();
					}
				}
			}
