...
import huntyboy102.moremod.client.TextureMapMO;
...
public class HoloIcons implements ITextureMapPopulator {
	...
	public HoloIcons() {
		iconMap = new HashMap<>();
		textureMap = new TextureMapMO("textures/gui/items", this);
	...
	public static void tessalateParticleIcon(TextureAtlasSprite icon, double x, double y, double z, float size,
			Color color) {
		RenderUtils.tessalateParticle(Minecraft.getMinecraft().getRenderViewEntity(), icon, size, new Vec3d(x, y, z),
				color);
	}

	public static void tessalateStaticIcon(TextureAtlasSprite icon, double x, double y, double z, float size,
			Color color) {
		tessalateStaticIcon(icon, x, y, z, size, color, 1);
	}

	public static void tessalateStaticIcon(TextureAtlasSprite icon, double x, double y, double z, float size,
			Color color, float multiply) {
		...
		wr.pos(x - halfSize, y - halfSize, z).tex(uMax, vMax).color(color.getFloatR() * multiply,
				color.getFloatG() * multiply, color.getFloatB() * multiply, color.getFloatA()).endVertex();
		wr.pos(x + halfSize, y - halfSize, z).tex(uMin, vMax).color(color.getFloatR() * multiply,
				color.getFloatG() * multiply, color.getFloatB() * multiply, color.getFloatA()).endVertex();
		wr.pos(x + halfSize, y + halfSize, z).tex(uMin, vMin).color(color.getFloatR() * multiply,
				color.getFloatG() * multiply, color.getFloatB() * multiply, color.getFloatA()).endVertex();
		wr.pos(x - halfSize, y + halfSize, z).tex(uMax, vMin).color(color.getFloatR() * multiply,
				color.getFloatG() * multiply, color.getFloatB() * multiply, color.getFloatA()).endVertex();
	}
	...
	public HoloIcon registerIcon(TextureMap textureMap, String iconName, int originalSize) {
		HoloIcon holoIcon = new HoloIcon(textureMap.registerSprite(new ResourceLocation(Reference.MOD_ID, iconName)),
