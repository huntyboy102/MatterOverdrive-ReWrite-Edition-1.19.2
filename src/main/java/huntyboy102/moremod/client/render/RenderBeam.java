...
package huntyboy102.moremod.client.render;/* Created by Simeon on 10/18/2015. */
...
public abstract class RenderBeam<T extends EntityLivingBase> implements IWorldLastRenderer {
	...
	protected boolean renderRaycastedBeam(Vec3d position, Vec3d direction, Vec3d offset, T caster) {
		...
		RayTraceResult hit = MOPhysicsHelper.rayTrace(position, caster.world, maxDistance, 0, new Vec3d(0, 0, 0), false,
				true, direction, caster);
		if (hit != null && hit.typeOfHit != RayTraceResult.Type.MISS) {
			renderBeam(position.add(direction.scale(0.2)), hit.hitVec, offset, getBeamColor(caster),
					getBeamTexture(caster), getBeamThickness(caster), caster);
			onBeamRender(caster);
			onBeamRaycastHit(hit, caster);
			return true;
		} else {
			renderBeam(position.add(direction.scale(0.2)),
					position.add(direction.x * maxDistance, direction.y * maxDistance, direction.z * maxDistance),
					offset, getBeamColor(caster), getBeamTexture(caster), getBeamThickness(caster), caster);
			onBeamRender(caster);
		}
		return false;
	}

	protected void renderBeam(Vec3d from, Vec3d to, Vec3d offest, Color color, ResourceLocation texture, float tickness,
			T viewer) {
		if (texture != null) {
