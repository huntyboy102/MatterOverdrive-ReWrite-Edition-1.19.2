...
public class BioticStatRendererShield implements IBioticStatRenderer<BioticStatShield> {
	...
	private static final ResourceLocation forcefield_damage_tex = new ResourceLocation(
			Reference.PATH_FX + "shield_damage.png");
	private static final ResourceLocation forcefield_tex = new ResourceLocation(
			Reference.PATH_FX + "forcefield_plasma.png");
	private static final ResourceLocation forcefield_plasma_tex = new ResourceLocation(
			Reference.PATH_FX + "forcefield_plasma_2.png");
	...
	public BioticStatRendererShield() {
		/*
		 * try { //shield_model = OBJLoader.instance.loadModel(model_path);
		 * //normal_sphere = OBJLoader.instance.loadModel(new
	...
	private void renderPlayerShield(RenderWorldLastEvent event, EntityPlayer player) {
		...
		if (isVisible) {
			...
			GlStateManager.translate(playerPosition.x - clientPosition.x, playerPosition.y - clientPosition.y,
					playerPosition.z - clientPosition.z);
			...
			GlStateManager.pushMatrix();
			GlStateManager.rotate((float) time * 0.005f, (float) Math.sin(time * 0.01), (float) Math.cos(time * 0.01),
					0);
	...
	private void renderAttacks(AndroidPlayer androidPlayer) {
		...
		/*
		 * if (androidPlayer.hasEffect(AndroidPlayer.NBT_HITS)) { NBTTagList hits =
		 * androidPlayer.getEffectTagList(AndroidPlayer.NBT_HITS, 10); for (int i = 0; i
		 * < hits.tagCount(); i++) { renderAttack(new
		 * Vector3f(hits.getCompoundTagAt(i).getFloat("x"),
		 * -hits.getCompoundTagAt(i).getFloat("y"),
		 * -hits.getCompoundTagAt(i).getFloat("z")).normalise(null),
		 * (hits.getCompoundTagAt(i).getInteger("time") / 10f) * opacity); } }
		 */
	}
