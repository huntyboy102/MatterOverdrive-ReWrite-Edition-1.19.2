...
public class DialogShotFromBehind extends DialogShot {
	...
	public boolean positionCamera(EntityLivingBase active, EntityLivingBase other, float ticks,
		...
		Vec3d pos = rendererConversation.getPosition(other, ticks).add((left.x * sideOffset) / lookDistance,
				(left.y * sideOffset) / lookDistance, (left.z * sideOffset) / lookDistance);
		RayTraceResult position = MOPhysicsHelper.rayTrace(pos, other.world, distance, ticks, null, true, false, look,
				other);
		if (position != null) {
			pos = position.hitVec;
		} else {
