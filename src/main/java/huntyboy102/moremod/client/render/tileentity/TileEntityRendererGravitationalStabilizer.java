...
public class TileEntityRendererGravitationalStabilizer
	...
	@Override
	public void render(TileEntityMachineGravitationalStabilizer stabilizer, double x, double y, double z, float ticks,
		...
		if (stabilizer.getHit() != null) {
			...
			Vector3f source = new Vector3f(stabilizer.getPos().getX(), stabilizer.getPos().getY(),
					stabilizer.getPos().getZ());
			...
			GlStateManager.color((float) stabilizer.getBeamColorR(), (float) stabilizer.getBeamColorG(),
					(float) stabilizer.getBeamColorB());
			bindTexture(beam);
			...
			GlStateManager.pushMatrix();
			GlStateManager.translate(dir.x * distance / 2, dir.y * distance / 2, dir.z * distance / 2);
			GlStateManager.scale((destination.x - source.x) + 1, (destination.y - source.y) + 1,
					(destination.z - source.z) + 1);
			GlStateManager.rotate((float) (omega * (180 / Math.PI)), c.x, c.y, c.z);
	...
	public void renderScreen(double x, double y, double z, TileEntityMachineGravitationalStabilizer stabilizer,
			TileEntityGravitationalAnomaly anomaly) {
