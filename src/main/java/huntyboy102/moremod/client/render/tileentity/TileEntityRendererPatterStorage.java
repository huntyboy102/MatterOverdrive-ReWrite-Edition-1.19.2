...
public class TileEntityRendererPatterStorage extends TileEntitySpecialRenderer<TileEntityMachinePatternStorage> {
	...
	@Override
	public void render(TileEntityMachinePatternStorage patternStorage, double x, double y, double z, float ticks,
			int destroyStage, float a) {
		/*
		 * GL11.glPushMatrix(); GlStateManager.translate(x + 0.5f, y + 0.5f, z + 0.5f);
		 * RenderUtils.rotateFromBlock(patternStorage.getWorld(),
		 * patternStorage.getPosition()); bindTexture(texture);
		 * 
		 * for (int i = 0; i < patternStorage.pattern_storage_slots.length; i++) {
		 * ItemStack drive =
		 * patternStorage.getStackInSlot(patternStorage.pattern_storage_slots[i]);
		 * if(drive != null) { GL11.glPushMatrix(); GL11.glTranslatef(i >= 3 ? -0.3f :
		 * 0.3f, 0.1f - 0.2f * (i % 3), -0.2f); //model.renderPart("drive");
