...
public abstract class TileEntityRendererStation<T extends MOTileEntityMachine> extends TileEntitySpecialRenderer<T>
	...
	protected Color getHoloColor(TileEntity entity) {
		if (((MOTileEntityMachine) entity).isUsableByPlayer(Minecraft.getMinecraft().player)) {
			return holoColor;
		}
	...
	public void render(T machine, double x, double y, double z, float ticks, int destroyStage, float a) {
		if (!machine.shouldRender())
			return;
		double t = MOMathHelper.noise(machine.getPos().getX() * 0.3, machine.getPos().getY() * 0.3,
				machine.getPos().getZ() * 0.3);
		GlStateManager.pushMatrix();
		GlStateManager.enableBlend();
		GlStateManager.blendFunc(GL_ONE, GL_ONE);
		try {
	...
	protected void rotate(T station, double noise) {
		GlStateManager.rotate((Minecraft.getMinecraft().world.getWorldTime() * 0.5f) + (1800 * (float) noise), 0, -1,
				0);
	}

	protected boolean isUsable(T station) {
		return (station).isUsableByPlayer(Minecraft.getMinecraft().player);
	}
	...
	public void onConfigChanged(ConfigurationHandler config) {
		enableHoloShader = config.getBool("use holo shader", ConfigurationHandler.CATEGORY_CLIENT, true,
				"Use the custom holo shader for holographic items");
	}
}
