...
public class StarMapRendererStar extends StarMapRendererAbstract {
	...
	public StarMapRendererStar() {
		...
		try {
			IModel model = OBJLoader.INSTANCE
					.loadModel(new ResourceLocation(Reference.PATH_MODEL + "block/sphere.obj"));
			star_model = model.bake(model.getDefaultState(), DefaultVertexFormats.ITEM,
	...
	public void renderBody(Galaxy galaxy, SpaceBody spaceBody, TileEntityMachineStarMap starMap, float partialTicks,
		...
		if (spaceBody instanceof Star) {
			...
			RenderUtils.tessalateParticle(Minecraft.getMinecraft().getRenderViewEntity(), star_icon, star.getSize(),
					new Vec3d(0, 0, 0), Reference.COLOR_HOLO_YELLOW.getFloatR() * 0.1f,
					Reference.COLOR_HOLO_YELLOW.getFloatG() * 0.1f, Reference.COLOR_HOLO_YELLOW.getFloatB() * 0.1f,
			...
			Tessellator.getInstance().draw();
			Tessellator.getInstance().getBuffer().begin(GL_QUADS, DefaultVertexFormats.ITEM);
			if (Minecraft.getMinecraft().world.getWorldTime() % 120 > 80) {
				float t = ((Minecraft.getMinecraft().world.getWorldTime() % 120) - 80) / 40f;
			...
			for (Planet planet : star.getPlanets()) {
				...
				GlStateManager.translate(Math.sin(time * 0.001 + 10 * planetID) * radius, 0,
				...
				if (starMap.getDestination().equals(planet)) {
					RenderUtils.tessalateParticle(Minecraft.getMinecraft().getRenderViewEntity(), selectedIcon,
							planet.getSize() * 0.15f * sizeMultiply, new Vec3d(0, 0, 0), planetColor);
				}

				if (starMap.getGalaxyPosition().equals(planet)) {
					RenderUtils.tessalateParticle(Minecraft.getMinecraft().getRenderViewEntity(), currentIcon,
							planet.getSize() * 0.25f, new Vec3d(0, 0, 0), planetColor);
				}
		...
		if (spaceBody instanceof Star) {
			...
			if (planet != null) {
				if (GalaxyClient.getInstance().canSeePlanetInfo(planet, Minecraft.getMinecraft().player)) {
					...
					RenderUtils.drawString(Minecraft.getMinecraft().standardGalacticFontRenderer,
							planet.getSpaceBodyName(), 72, -42, Reference.COLOR_HOLO, opacity);
				}

				RenderUtils.applyColorWithMultipy(Reference.COLOR_HOLO, opacity);
				ClientProxy.holoIcons.renderIcon("icon_size", 72, -28);
				RenderUtils.drawString(DecimalFormat.getPercentInstance().format(planet.getSize()), 92, -23,
	...
	private void drawPlanetInfo(Planet planet) {
		...
		if (GalaxyClient.getInstance().canSeePlanetInfo(planet, Minecraft.getMinecraft().player)) {
			int width = Minecraft.getMinecraft().fontRenderer.getStringWidth(planet.getSpaceBodyName());
			Minecraft.getMinecraft().fontRenderer.drawString(planet.getSpaceBodyName(), -width / 2, 0,
					Planet.getGuiColor(planet).getColor());

			if (planet.isHomeworld(Minecraft.getMinecraft().player)) {
				width = Minecraft.getMinecraft().fontRenderer.getStringWidth("[Home]");
				Minecraft.getMinecraft().fontRenderer.drawString(TextFormatting.GOLD + "[Home]", -width / 2, -10,
						0xFFFFFF);
			}
		} else {
			int width = Minecraft.getMinecraft().standardGalacticFontRenderer.getStringWidth(planet.getSpaceBodyName());
			Minecraft.getMinecraft().standardGalacticFontRenderer.drawString(planet.getSpaceBodyName(), -width / 2, 0,
					Planet.getGuiColor(planet).getColor());

			if (planet.hasOwner()) {
				EntityPlayer owner = Minecraft.getMinecraft().world.getPlayerEntityByUUID(planet.getOwnerUUID());
				if (owner != null) {
					...
					Minecraft.getMinecraft().fontRenderer.drawString(TextFormatting.GOLD + info, -width / 2, -10,
							0xFFFFFF);
				}
			}
		}
	...
	private void drawPlanetOrbit(Planet planet, double radius) {
		...
		wr.color(Planet.getGuiColor(planet).getFloatR() * 0.1f, Planet.getGuiColor(planet).getFloatG() * 0.1f,
				Planet.getGuiColor(planet).getFloatB() * 0.1f, Planet.getGuiColor(planet).getFloatA() * 0.1f);
		for (int i = 0; i < 32; i++) {
