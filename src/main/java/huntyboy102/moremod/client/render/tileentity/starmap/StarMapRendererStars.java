...
public abstract class StarMapRendererStars extends StarMapRendererAbstract {
	...
	protected void renderStars(Quadrant quadrant, TileEntityMachineStarMap starMap, double distanceMultiply,
                               double starSizeMultiply) {
		...
		if (quadrant != null) {
			...
			for (Star star : quadrant.getStars()) {
				pos = new Vec3d(star.getPosition().x * distanceMultiply, star.getPosition().y * distanceMultiply,
						star.getPosition().z * distanceMultiply);
	...
	protected void drawStarParticle(Quadrant quadrant, Star star, Vec3d pos, EntityPlayer player,
			TileEntityMachineStarMap starMap, double starSizeMultiply) {
		...
		if (starMap.getDestination().equals(star)) {
			...
			RenderUtils.tessalateParticle(Minecraft.getMinecraft().getRenderViewEntity(), selectedIcon,
					star.getSize() * 0.05 * starSizeMultiply, pos, color);
		}
		if (starMap.getGalaxyPosition().equals(star)) {
			size = 0.035;
		...
		RenderUtils.tessalateParticle(Minecraft.getMinecraft().getRenderViewEntity(), star_icon,
				star.getSize() * size * starSizeMultiply, pos, color);
	}
}
