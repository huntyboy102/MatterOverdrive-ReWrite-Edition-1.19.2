...
import huntyboy102.moremod.client.render.weapons.layers.IWeaponLayer;
import huntyboy102.moremod.client.render.weapons.modules.IModuleRender;
...
public class WeaponRenderHandler {
	...
	public void onHandRender(RenderSpecificHandEvent event) {
		...
		if (event.getHand() == EnumHand.MAIN_HAND && !weapon.isEmpty() && weapon.getItem() instanceof EnergyWeapon) {
			...
			Project.gluPerspective(35f, (float) mc.displayWidth / (float) mc.displayHeight, 0.05F,
					(float) (mc.gameSettings.renderDistanceChunks * 32));
			...
			if (this.mc.gameSettings.thirdPersonView == 0 && !flag && !this.mc.gameSettings.hideGUI
				...
				float recoilValue = MOEasing.Quad.easeInOut(ClientWeaponHandler.RECOIL_TIME, 0, 1, 1f)
						* MathHelper.clamp(ClientWeaponHandler.RECOIL_AMOUNT, 0, 20);
				...
				if (model != null) {
					...
					transformFromModules(modules, model.getWeaponMetadata(), weapon, event.getPartialTicks(),
							zoomValue);
	...
	public void handleCameraRecoil(EntityViewRenderEvent.CameraSetup event) {
		event.setRoll(
				event.getRoll() + ClientWeaponHandler.CAMERA_RECOIL_AMOUNT * ClientWeaponHandler.CAMERA_RECOIL_TIME);
		event.setPitch(event.getPitch()
				+ Math.abs(ClientWeaponHandler.CAMERA_RECOIL_AMOUNT) * ClientWeaponHandler.CAMERA_RECOIL_TIME * 0.5f);
	}
