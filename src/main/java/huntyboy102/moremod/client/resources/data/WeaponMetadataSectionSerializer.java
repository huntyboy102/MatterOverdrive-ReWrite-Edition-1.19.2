...
package huntyboy102.moremod.client.resources.data;
...
public class WeaponMetadataSectionSerializer extends BaseMetadataSectionSerializer
	...
	@Override
	public WeaponMetadataSection deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
		...
		try {
			...
			if (modules != null) {
				for (Map.Entry<String, JsonElement> element : modules.entrySet()) {
					weaponMetadataSection.setModulePosition(element.getKey(),
							fromJson(element.getValue().getAsJsonObject()));
				}
			}
		} catch (ClassCastException classcastexception) {
			throw new JsonParseException(
					"Invalid weapon->scope_position: expected array, was " + jsonobject.get("scope_position"),
					classcastexception);
		}
