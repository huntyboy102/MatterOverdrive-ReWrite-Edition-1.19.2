...
public class MOCommand extends CommandBase {
	...
	public void execute(MinecraftServer minecraftServer, ICommandSender iCommandSender, String[] args)
		...
		if (args.length == 0) {
			...
			iCommandSender.sendMessage(new TextComponentString(
					"Format: /matterregistry <recalculate/blacklist/register> <item/itemstack/ore> <matter value> <player(optional)>"));
			iCommandSender.sendMessage(new TextComponentString("Example: /matterregistry register 3000 "));
			iCommandSender.sendMessage(new TextComponentString(
					"Will register the item you are holding with a matter value of 3000, Specifing a Player name will only set a value for that player."));
			return;
		}
