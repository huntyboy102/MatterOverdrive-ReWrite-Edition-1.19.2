...
public class ContainerMachine<T extends MOTileEntityMachine> extends MOBaseContainer implements IMachineWatcher {
	...
	protected boolean putInPlayerInventory(ItemStack itemStack) {
		return MOInventoryHelper.mergeItemStack(inventorySlots, itemStack, machine.getSizeInventory(),
				inventorySlots.size() - machine.getSizeInventory(), true, true);
	}

	protected boolean tryAndPutInMachineSlots(ItemStack itemStack, IInventory inventory) {
		return MOInventoryHelper.mergeItemStack(inventorySlots, itemStack, 0, inventory.getSizeInventory(), false,
				true);
	}
	...
	public boolean isWatcherValid() {
		if (entityPlayer instanceof EntityPlayerMP) {
			return ((EntityPlayerMP) entityPlayer).getServerWorld()
					.getPlayerEntityByUUID(entityPlayer.getUniqueID()) != null;
		}
		return false;
	}
}
