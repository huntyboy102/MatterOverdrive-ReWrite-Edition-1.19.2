...
public class EntityAIFollowCreator<T extends EntityLiving & IEntityOwnable> extends EntityAIBase {
	...
	public boolean continueExecuting() {
		return !this.petPathfinder.noPath()
				&& this.pet.getDistanceSq(this.theOwner) > (double) (this.maxDist * this.maxDist);
	}
	...
	public void updateTask() {
		this.pet.getLookHelper().setLookPositionWithEntity(this.theOwner, 10.0F,
				(float) this.pet.getVerticalFaceSpeed());

		if (--this.timeToRecalcPath <= 0) {
			this.timeToRecalcPath = 10;

			if (!this.petPathfinder.tryMoveToXYZ(this.theOwner.posX, theOwner.posY + theOwner.getEyeHeight(),
					theOwner.posZ + 2, this.followSpeed)) {
					...
					if (this.pet.getDistanceSq(this.theOwner) >= 144.0D) {
						...
						for (int xMov = 0; xMov <= 3; ++xMov) {
							for (int zMov = 0; zMov <= 3; ++zMov) {
								if ((xMov < 1 || zMov < 1 || xMov > 2 || zMov > 2)
										&& this.func_181065_a(new BlockPos(x + xMov, y, z + zMov))
										&& this.func_181065_a(new BlockPos(x + xMov, y + 1, z + zMov))) {
									this.pet.setLocationAndAngles((double) ((float) (x + xMov) + 0.5F), (double) y,
											(double) ((float) (z + zMov) + 0.5F), this.pet.rotationYaw,
