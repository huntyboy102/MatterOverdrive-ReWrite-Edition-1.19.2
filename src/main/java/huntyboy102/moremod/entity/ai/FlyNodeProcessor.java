...
public class FlyNodeProcessor extends NodeProcessor {
	...
	public PathPoint getPathPointTo(Entity entity) {
		return this.openPoint(MathHelper.floor(entity.getEntityBoundingBox().minX),
				MathHelper.floor(entity.getEntityBoundingBox().maxY + 0.5),
	...
	public PathPoint getPathPointTos(Entity entity, double x, double y, double target) {
		return this.openPoint(MathHelper.floor(x - (double) (entity.width / 2.0F)), MathHelper.floor(y + 0.5D),
				MathHelper.floor(target - (double) (entity.width / 2.0F)));
	}
	...
	public int findPathOptions(PathPoint[] pathOptions, Entity entity, PathPoint currentPoint, PathPoint targetPoint,
		...
		for (EnumFacing enumfacing : EnumFacing.values()) {
			PathPoint pathpoint = this.func_186328_b(currentPoint.x + enumfacing.getXOffset(),
					currentPoint.y + enumfacing.getYOffset(), currentPoint.z + enumfacing.getZOffset());
	...
	@Override
	public PathNodeType getPathNodeType(IBlockAccess world, int x, int y, int z, EntityLiving entity, int xSize,
	...
	public PathPoint getPathPointToCoords(double x, double y, double z) {
		return this.openPoint(MathHelper.floor(x - (double) (this.entity.width / 2.0F)), MathHelper.floor(y + 0.5D),
				MathHelper.floor(z - (double) (this.entity.width / 2.0F)));
	}
