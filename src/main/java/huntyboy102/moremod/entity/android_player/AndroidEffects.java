...
public class AndroidEffects {
	...
	public static void readEffectFromNBT(NBTTagCompound tagCompound, Effect effect) {
		...
		case 8:
			...
			effect.value = new BlockPos(((NBTTagInt) blockTag.get(0)).getInt(), ((NBTTagInt) blockTag.get(1)).getInt(),
					((NBTTagInt) blockTag.get(2)).getInt());
			break;
		case 9:
	...
	public <T> void updateEffect(int id, T newData) {
		...
		if (!typeMap.get(newData.getClass()).equals(effect.typeId)) {
			throw new ClassCastException(
					String.format("Class: %s of value not the same as in stored effect", newData.getClass().getName()));
		}
