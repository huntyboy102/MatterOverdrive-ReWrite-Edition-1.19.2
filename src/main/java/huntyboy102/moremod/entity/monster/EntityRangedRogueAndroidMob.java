...
import huntyboy102.moremod.entity.ai.AndroidTargetSelector;
import huntyboy102.moremod.entity.ai.EntityAIMoveAlongPath;
import huntyboy102.moremod.entity.ai.EntityAIPhaserBoltAttack;
import huntyboy102.moremod.entity.ai.EntityAIRangedRunFromMelee;
import huntyboy102.moremod.items.android.RougeAndroidParts;
...
public class EntityRangedRogueAndroidMob extends EntityRougeAndroidMob implements IRangedEnergyWeaponAttackMob {
		...
		if (EntityRogueAndroid.dropItems)
			if (!hasTeam() || recentlyHit) {
				...
				if (rand.nextFloat() < (0.15f + lootingModifier) || getIsLegendary()) {
					ItemStack part = MatterOverdrive.ANDROID_PARTS_FACTORY.generateRandomDecoratedPart(
	...
	public void addRandomArmor() {
		...
		ItemStack gun = MatterOverdrive.WEAPON_FACTORY.getRandomDecoratedEnergyWeapon(
				new WeaponFactory.WeaponGenerationContext(androidLevel, this, getIsLegendary()));
		setItemStackToSlot(EntityEquipmentSlot.MAINHAND, gun);
		this.getEntityAttribute(SharedMonsterAttributes.FOLLOW_RANGE)
				.setBaseValue(((EnergyWeapon) gun.getItem()).getRange(gun) - 2);
	}
	...
	public void attackEntityWithRangedAttack(EntityLivingBase target, Vec3d lastSeenPosition, boolean canSee) {
		...
		if (!world.isRemote) {
			...
			if (weapon.getItem() instanceof EnergyWeapon) {
				...
				float difficulty = MathHelper.clamp((0.6f / 3f) * world.getDifficulty().getId(), 0, 0.6f)
						+ getAndroidLevel() * (0.4f / 3f) + (getIsLegendary() ? 0.3f : 0);
				...
				MatterOverdrive.NETWORK.sendToAllAround(new PacketFirePlasmaShot(this.getEntityId(), pos, dir, shot),
						world.provider.getDimension(), posX, posY, posZ, 64);

				difficulty = 1 + (3 - world.getDifficulty().getId()) * 0.5f;
				this.aiBoltAttack.setMaxRangedAttackDelay(
						(int) (((EnergyWeapon) weapon.getItem()).getShootCooldown(weapon) * difficulty));
			}
		}
	}
	 ...
	 */
	/*
	 * @Override public void setCurrentItemOrArmor(int slot, ItemStack itemStack) {
	 * super.setCurrentItemOrArmor(slot, itemStack);
	 * 
	 * if (!this.world.isRemote && slot == 0) { this.setCombatTask(); } }
	 */
	@Override
