...
package huntyboy102.moremod.entity.monster;
...
public class EntityRogueAndroid implements IConfigSubscriber {
	...
	private static void loadBiomeBlacklist(ConfigurationHandler config) {
		...
		String[] blacklist = config.config.getStringList("biome.blacklist",
				ConfigurationHandler.CATEGORY_ENTITIES + ".rogue_android", new String[] { "minecraft:hell",
						"minecraft:sky", "minecraft:mushroom_island", "minecraft:mushroom_island_shore" },
				"Rogue Android biome blacklist");
	...
	private static void loadBiomesWhitelist(ConfigurationHandler configurationHandler) {
		...
		String[] whitelist = configurationHandler.config.getStringList("biome.whitelist",
	...
	private static void loadDimensionBlacklist(ConfigurationHandler configurationHandler) {
		...
		Property blacklistProp = configurationHandler.config.get(
				ConfigurationHandler.CATEGORY_ENTITIES + "." + "rogue_android", "dimension.blacklist", new int[] { 1 });
	...
	private static void loadDimesionWhitelist(ConfigurationHandler configurationHandler) {
		...
		Property whitelistProp = configurationHandler.config
	...
	public void onConfigChanged(ConfigurationHandler config) {
		...
		EntityRangedRogueAndroidMob.UNLIMITED_WEAPON_ENERGY = config.getBool("unlimited_weapon_energy", category, true,
				"Do Ranged Rogue Androids have unlimited weapon energy in their weapons");
		MAX_ANDROIDS_PER_CHUNK = config.getInt("max_android_per_chunk", category, 4,
				"The max amount of Rogue Android that can spawn in a given chunk");
		SPAWN_CHANCE = config.config.getFloat("spawn_chance_percent", category, 0.1f, 0, 1,
				"The spawn chance of rogue androids. How likely are they to spawn once chosen to spawn.");
		EntityRangedRogueAndroidMob.DROP_NORMAL_WEAPONS = config.getBool("drop_weapons", category, true,
				"Should normal Rogue Androids drop their weapons? If set to false they will never drop their weapons, but if set to true there is a small chance they will drop them.");
		EntityRangedRogueAndroidMob.DROP_LEGENDARY_WEAPONS = config.getBool("drop_legendary_weapons", category, true,
				"Should Legendary rogue androids drop Legendary weapons");
		LEGENDARY_SPAWN_CHANCE = config.config.getFloat("legendary_spawn_chance_percent", category, 0.03f, 0, 1,
				"The chance in percent, of rogue androids becoming legendary. This is the base value. This value is multiplied by the android's level");
