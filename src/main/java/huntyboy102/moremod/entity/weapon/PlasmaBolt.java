...
package huntyboy102.moremod.entity.weapon;

import java.util.List;

import io.netty.buffer.ByteBuf;
import matteroverdrive.MatterOverdrive;
import huntyboy102.moremod.api.events.weapon.MOEventPlasmaBlotHit;
...
import huntyboy102.moremod.fx.EntityFXGenericAnimatedParticle;
...
public class PlasmaBolt extends Entity implements IProjectile, IGravityEntity, IEntityAdditionalSpawnData {
	...
	public PlasmaBolt(World world, EntityLivingBase entityLivingBase, Vec3d position, Vec3d dir, WeaponShot shot,
			float speed) {
		super(world);
	...
	public void onUpdate() {
		if (!world.isRemote && shootingEntity instanceof EntityPlayerMP) {
			MatterOverdrive.NETWORK.sendTo(new PacketUpdatePlasmaBolt(getEntityId(), posX, posY, posZ),
			...
			if (movingobjectposition.entityHit != null) {
				...
				if (world.isRemote) {
					if (weapon != null && weapon.getItem() instanceof EnergyWeapon) {
						((EnergyWeapon) weapon.getItem()).onProjectileHit(movingobjectposition, weapon, world, 5);
					}
					onHit(movingobjectposition);
				}
				manageExplosions(movingobjectposition);
				MinecraftForge.EVENT_BUS.post(new MOEventPlasmaBlotHit(weapon, movingobjectposition, this,
		...
		// this.posX = (this.getEntityBoundingBox().minX +
		// this.getEntityBoundingBox().maxX) / 2.0D;
		// this.posY = this.getEntityBoundingBox().minY + this.getYOffset() -
		// (double)this.height;
		// this.posZ = (this.getEntityBoundingBox().minZ +
		// this.getEntityBoundingBox().maxZ) / 2.0D;
	...
	private boolean manageExplosions(RayTraceResult hit) {
		if (explodeMultiply > 0) {
			if (!world.isRemote) {
				Explosion explosion = new Explosion(world, this, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z,
						explodeMultiply, false, false);
				if (!net.minecraftforge.event.ForgeEventFactory.onExplosionStart(world, explosion)) {
					explosion.doExplosionA();
					this.world.playSound(null, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z,
