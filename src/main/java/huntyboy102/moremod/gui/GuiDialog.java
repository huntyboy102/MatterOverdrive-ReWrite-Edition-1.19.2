...
import huntyboy102.moremod.api.dialog.IDialogMessageSeedable;
...
public class GuiDialog extends GuiScreen {
	public static final ResourceLocation separator_texture = new ResourceLocation(
			Reference.PATH_ELEMENTS + "dialog_separator.png");
	...
	public void drawDialog(int mouseX, int mouseY, float ticks) {
		...
		if (message != null && !message.isEmpty()) {
			...
			for (int i = 0; i < splitMessage.size(); i++) {
				...
				fontRenderer.drawString(
						m, width / 2 - messageWidth - 16, height - height / 8 - 64
								- (splitMessage.size() * fontRenderer.FONT_HEIGHT / 2) + fontRenderer.FONT_HEIGHT * i,
		...
		for (IDialogOption option : options) {
			...
			if (option.isVisible(npc, player)) {
				...
				if (mouseX > messageX && mouseX <= messageX + messageWidth && mouseY > messageY
						&& mouseY <= messageY + fontRenderer.FONT_HEIGHT) {
					GlStateManager.disableTexture2D();
					GlStateManager.blendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
					GlStateManager.color(0, 0, 0, 0.4f);
					drawModalRectWithCustomSizedTexture(messageX - 2, messageY - 4, 0, 0, messageWidth + 10,
							fontRenderer.FONT_HEIGHT + 8, messageWidth + 8, fontRenderer.FONT_HEIGHT + 8);
					if (canInteract) {
						GlStateManager.color(1, 1, 1, 0.6f);
					} else {
						RenderUtils.applyColor(Reference.COLOR_HOLO_RED);
					}
					drawModalRectWithCustomSizedTexture(messageX - 2 + messageWidth + 10, messageY - 4, 0, 0, 2,
							fontRenderer.FONT_HEIGHT + 8, 2, fontRenderer.FONT_HEIGHT + 8);
					GlStateManager.enableTexture2D();

					if (option.getHoloIcon(npc, player) != null) {
						GlStateManager.color(1, 1, 1);
						ClientProxy.holoIcons.renderIcon(option.getHoloIcon(npc, player), messageX - 18,
								messageY + fontRenderer.FONT_HEIGHT / 2 - 8, 16, 16);
					}
					fontRenderer.drawString(optionText, messageX + 2, messageY,
							canInteract ? 0xFFFFFF : Reference.COLOR_HOLO_RED.getColor());
				} else {
					if (option.getHoloIcon(npc, player) != null) {
					...
					fontRenderer.drawString(optionText, messageX, messageY,
							canInteract ? Reference.COLOR_HOLO.getColor() : Reference.COLOR_HOLO_RED.getColor());
				}
				optionPos++;
			}
		}
