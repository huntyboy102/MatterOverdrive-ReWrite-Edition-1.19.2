...
import huntyboy102.moremod.util.animation.MOAnimationTimeline;
...
public class GuiQuestHud {
	...
	public GuiQuestHud() {
		...
		completeQuestTimeline.addSlice(new MOAnimationTimeline.Slice(0, 1, 0, QUEST_FADE_IN_TIME,
				new MOAnimationTimeline.Easing.QuadEaseIn()));
		completeQuestTimeline.addSlice(new MOAnimationTimeline.Slice(1, 1, QUEST_FADE_IN_TIME,
				QUEST_NOTIFICATION_TIME - QUEST_FADE_OUT_TIME, null));
		completeQuestTimeline
				.addSlice(new MOAnimationTimeline.Slice(1, 0, QUEST_NOTIFICATION_TIME - QUEST_FADE_OUT_TIME,
						QUEST_FADE_OUT_TIME, new MOAnimationTimeline.Easing.QuadEaseOut()));
		...
		startedQuestTimeline.addSlice(new MOAnimationTimeline.Slice(0, 1, 0, QUEST_FADE_IN_TIME,
				new MOAnimationTimeline.Easing.QuadEaseIn()));
		startedQuestTimeline.addSlice(new MOAnimationTimeline.Slice(1, 1, QUEST_FADE_IN_TIME,
				QUEST_NOTIFICATION_TIME - QUEST_FADE_OUT_TIME, null));
		startedQuestTimeline.addSlice(new MOAnimationTimeline.Slice(1, 0, QUEST_NOTIFICATION_TIME - QUEST_FADE_OUT_TIME,
				OBJECTIVES_FADE_TIME, new MOAnimationTimeline.Easing.QuadEaseOut()));
		...
		objectivesTimeline.addSlice(new MOAnimationTimeline.Slice(0, 1, 0, OBJECTIVES_FADE_TIME,
				new MOAnimationTimeline.Easing.QuadEaseIn()));
		objectivesTimeline.addSlice(new MOAnimationTimeline.Slice(1, 1, QUEST_FADE_IN_TIME,
				QUEST_NOTIFICATION_TIME - QUEST_FADE_OUT_TIME, null));
		objectivesTimeline
				.addSlice(new MOAnimationTimeline.Slice(1, 0, OBJECTIVES_NOTIFICATION_TIME - OBJECTIVES_FADE_TIME,
						OBJECTIVES_FADE_TIME, new MOAnimationTimeline.Easing.QuadEaseOut()));
	}
	...
	public void onRenderOverlay(RenderGameOverlayEvent.Post event) {
		if (event.getType().equals(RenderGameOverlayEvent.ElementType.ALL)) {
			...
			if (completeQuestTimeline.isPlaying() && completeQuestName != null) {
				...
				Color color = new Color(Reference.COLOR_HOLO.getIntR(), Reference.COLOR_HOLO.getIntG(),
						Reference.COLOR_HOLO.getIntB(), 20 + (int) (235f * time));
				...
				GlStateManager.translate(event.getResolution().getScaledWidth() - titleWidth - 30 - time * 30, y - 20,
						0);
				GlStateManager.scale(1.5, 1.5, 1.5);
				fontRenderer.drawStringWithShadow(TextFormatting.BOLD + completeQuestName, 0, 40, color.getColor());
				GlStateManager.popMatrix();
				fontRenderer.drawStringWithShadow("Completed:",
						event.getResolution().getScaledWidth() - titleWidth - 20 - (int) (time * 40), y + 28,
						color.getColor());
				if (completeQuestXp > 0) {
					fontRenderer.drawStringWithShadow(String.format("+%dxp", (int) (time * completeQuestXp)),
							event.getResolution().getScaledWidth() - 50 - (int) (20 * time), y + 58, color.getColor());
				}
			}
			if (startedQuestTimeline.isPlaying() && newQuestName != null) {
				float time = startedQuestTimeline.getCurrentValue();
				Color color = new Color(Reference.COLOR_HOLO.getIntR(), Reference.COLOR_HOLO.getIntG(),
						Reference.COLOR_HOLO.getIntB(), 19 + (int) (235f * time));
				GlStateManager.pushMatrix();
			...
			if (objectivesTimeline.isPlaying() && objectiveChanged != null) {
				...
				if (objectiveChanged != null) {
					fontRenderer.drawStringWithShadow(String.format("[ %s ]", objectiveChanged), (int) (time * 20),
							(int) (event.getResolution().getScaledHeight() * 0.5) + objectivesY, color.getColor());
	...
	public void addObjectivesChanged(QuestStack oldQuestStack, QuestStack newQestStack, QuestState questState) {
		for (int i = 0; i < questState.getObjectiveIds().length; i++) {
			objectivesChangedQueue
					.add(newQestStack.getObjective(Minecraft.getMinecraft().player, questState.getObjectiveIds()[i]));
		}
