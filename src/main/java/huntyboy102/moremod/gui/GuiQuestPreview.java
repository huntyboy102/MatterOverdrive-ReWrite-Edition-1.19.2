...
public class GuiQuestPreview extends MOGuiBase {
	...
	private void loadQuestInfo(QuestStack questStack) {
		...
		for (int i = 0; i < questStack.getObjectivesCount(Minecraft.getMinecraft().player); i++) {
			List<String> objectiveLines = MatterOverdrive.QUEST_FACTORY.getFormattedQuestObjective(
					Minecraft.getMinecraft().player, questStack, i, width, TextFormatting.BLUE.toString(),
					TextFormatting.BLUE.toString());
			questInfo.addLines(objectiveLines);
		}
		questInfo.addLine("");
		questInfo.addLine(TextFormatting.DARK_PURPLE + "Rewards:");
		questInfo.addLine(String.format(TextFormatting.DARK_PURPLE + "   +%sxp",
				questStack.getXP(Minecraft.getMinecraft().player)));
		...
		for (int i = 0; i < rewards.size(); i++) {
			if (rewards.get(i) instanceof ItemStackReward && rewards.get(i).isVisible(questStack)) {
				ElementItemPreview itemPreview = new ElementItemPreview(this, i * 20, 1,
	...
	public void handleElementButtonClick(MOElementBase element, String elementName, int mouseButton) {
		...
		if (element == acceptButton) {
			MatterOverdrive.NETWORK.sendToServer(new PacketQuestActions(PacketQuestActions.QUEST_ACTION_ADD,
					mc.player.inventory.currentItem, mc.player));
