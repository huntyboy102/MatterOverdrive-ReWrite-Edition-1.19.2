...
import huntyboy102.moremod.gui.pages.starmap.PageGalaxy;
import huntyboy102.moremod.gui.pages.starmap.PagePlanetMenu;
import huntyboy102.moremod.gui.pages.starmap.PageQuadrant;
import huntyboy102.moremod.gui.pages.starmap.PageStar;
...
public class GuiStarMap extends MOGuiMachine<TileEntityMachineStarMap> {
	public static ScaleTexture BG = new ScaleTexture(new ResourceLocation(Reference.PATH_GUI + "star_map.png"), 255,
			141).setOffsets(213, 34, 42, 94);
	...
	public GuiStarMap(InventoryPlayer inventoryPlayer, TileEntityMachineStarMap machine) {
		...
		sidePannel.BACKGROUND_TEXTURE
				.setLocation(new ResourceLocation(Reference.PATH_ELEMENTS + "right_side_bar_panel_bg_holo.png"));
	...
	public void registerPages(MOBaseContainer container, TileEntityMachineStarMap starMap) {
		...
		AddPage(pageGalaxy, ClientProxy.holoIcons.getIcon("page_icon_galaxy"), I18n.format("gui.tooltip.page.galaxy"))
				.setIconColor(Reference.COLOR_MATTER);
		pageQuadrant = new PageQuadrant(this, 0, 0, width, height, starMap);
		pageQuadrant.setName("Quadrant");
		AddPage(pageQuadrant, ClientProxy.holoIcons.getIcon("page_icon_quadrant"),
				I18n.format("gui.tooltip.page.quadrant")).setIconColor(Reference.COLOR_MATTER);
		pageStar = new PageStar(this, 0, 0, width, height, starMap);
		pageStar.setName("Star");
		AddPage(pageStar, ClientProxy.holoIcons.getIcon("page_icon_star"), I18n.format("gui.tooltip.page.star"))
				.setIconColor(Reference.COLOR_MATTER);
		planetPage = new PagePlanetMenu(this, 0, 0, width, height, (ContainerStarMap) container, starMap);
		planetPage.setName("Planet");
		AddPage(planetPage, ClientProxy.holoIcons.getIcon("page_icon_planet"), I18n.format("gui.tooltip.page.planet"))
				.setIconColor(Reference.COLOR_MATTER);
			...
			if (renderers != null) {
				for (ISpaceBodyHoloRenderer renderer : renderers) {
					if (renderer.displayOnZoom(machine.getZoomLevel(), machine.getActiveSpaceBody())) {
						...
						if (machine.getActiveSpaceBody() != null) {
							renderer.renderGUIInfo(GalaxyClient.getInstance().getTheGalaxy(),
									machine.getActiveSpaceBody(), machine, partialTick, 0.8f);
						}
	...
	public void drawWorldBackground(int p_146270_1_) {
		...
		if (machine.getZoomLevel() <= 2) {
			rotation = mc.world.getWorldTime() * 0.1f;
		}
		Minecraft.getMinecraft().getRenderViewEntity().rotationYaw = 180 + rotation;
		Minecraft.getMinecraft().getRenderViewEntity().prevRotationPitch = Minecraft.getMinecraft()
				.getRenderViewEntity().rotationPitch = 15;
		...
		Minecraft.getMinecraft().getRenderViewEntity().prevRotationPitch = Minecraft.getMinecraft()
				.getRenderViewEntity().rotationPitch = lastRotationPitch;
	...
	public void onPageChange(int newPage) {
		...
		if (newPage != machine.getZoomLevel()) {
			MatterOverdrive.NETWORK.sendToServer(new PacketStarMapClientCommands(machine, newPage,
					machine.getGalaxyPosition(), machine.getDestination()));
		}
	}
	...
	public void initGui() {
		...
		AddMainPlayerSlots(inventorySlots, this, "holo_with_bg", new Color(Reference.COLOR_HOLO.getIntR() / 3,
				Reference.COLOR_HOLO.getIntG() / 3, Reference.COLOR_HOLO.getIntB() / 3), 45, ySize - 104);
		AddHotbarPlayerSlots(inventorySlots, this, "holo_with_bg", new Color(Reference.COLOR_HOLO.getIntR() / 2,
				Reference.COLOR_HOLO.getIntG() / 2, Reference.COLOR_HOLO.getIntB() / 2), 45, ySize - 25);
	}
