...
public class GuiTransporter extends MOGuiMachine<TileEntityMachineTransporter> {
	...
	public GuiTransporter(InventoryPlayer inventoryPlayer, TileEntityMachineTransporter machine) {
		...
		matterStored = new ElementMatterStored(this, xSize - 35, 100,
	...
	private void updateInfo() {
		...
		xs.setBounds(machine.getPos().getX() - machine.getTransportRange(),
				machine.getPos().getX() + machine.getTransportRange());
		ys.setBounds(machine.getPos().getY() - machine.getTransportRange(),
				machine.getPos().getY() + machine.getTransportRange());
		zs.setBounds(machine.getPos().getZ() - machine.getTransportRange(),
				machine.getPos().getZ() + machine.getTransportRange());
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (pages.get(1).isVisible()) {
			getFontRenderer().drawString("X:", pages.get(1).getPosX() + xs.getPosX() - 10,
					pages.get(1).getPosY() + xs.getPosY() + 4, 0xFFFFFF);
			getFontRenderer().drawString(Integer.toString(xs.getNumber() - machine.getPos().getX()),
					pages.get(1).getPosX() + xs.getPosX() + xs.getWidth() + 4,
					pages.get(1).getPosY() + xs.getPosY() + 4, 0xFFFFFF);

			getFontRenderer().drawString("Y:", pages.get(1).getPosX() + ys.getPosX() - 10,
					pages.get(1).getPosY() + ys.getPosY() + 4, 0xFFFFFF);
			getFontRenderer().drawString(Integer.toString(ys.getNumber() - machine.getPos().getY()),
					pages.get(1).getPosX() + ys.getPosX() + ys.getWidth() + 4,
					pages.get(1).getPosY() + ys.getPosY() + 4, 0xFFFFFF);

			getFontRenderer().drawString("Z:", pages.get(1).getPosX() + zs.getPosX() - 10,
					pages.get(1).getPosY() + zs.getPosY() + 4, 0xFFFFFF);
			getFontRenderer().drawString(Integer.toString(zs.getNumber() - machine.getPos().getZ()),
					pages.get(1).getPosX() + zs.getPosX() + zs.getWidth() + 4,
					pages.get(1).getPosY() + zs.getPosY() + 4, 0xFFFFFF);
		}
	}
		...
		if (buttonName.equals("Import")) {
			...
			if (usb != null) {
				if (MatterOverdrive.ITEMS.transportFlashDrive.hasTarget(usb)) {
					machine.setSelectedLocation(MatterOverdrive.ITEMS.transportFlashDrive.getTarget(usb).add(0, 1, 0),
							name.getText());
	...
	public void textChanged(String elementName, String text, boolean typed) {
		if (typed) {
			if (elementName.equals("LocationName")) {
				machine.setSelectedLocation(new BlockPos(xs.getNumber(), ys.getNumber(), zs.getNumber()),
						name.getText());
				machine.sendConfigsToServer(true);
				// updateInfo();
			}
