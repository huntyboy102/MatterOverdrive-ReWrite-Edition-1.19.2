...
public abstract class MOGuiBase extends GuiContainer
		implements IButtonHandler, ITextHandler, IListHandler, GuiElementList {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTick, int x, int y) {
		...
		// RenderUtils.drawSizeableBackground(guiLeft, guiTop, xSize, ySize, texW, texH,
		// texture, this.zLevel, 57);
		if (background != null) {
	...
	public void drawTooltip(List<String> tooltips) {
		this.drawTooltipHoveringText(tooltips, this.mouseX + this.guiLeft, this.mouseY + this.guiTop,
				this.fontRenderer);
		this.tooltip.clear();
	}
	...
	public void handleMouseInput() throws IOException {
		...
		if (var3 != 0) {
			...
			while (var4-- > 0) {
				...
				if (var5.isVisible() && var5.isEnabled() && var5.intersectsWith(this.mouseX, this.mouseY)
						&& var5.onMouseWheel(this.mouseX, this.mouseY, var3)) {
					return;
				}
			}
		}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		} while (!var5.isVisible() || !var5.isEnabled() || !var5.intersectsWith(mouseX, mouseY)
				|| !var5.onMousePressed(mouseX, mouseY, mouseButton));
	...
	protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
		/*
		 * Slot var6 = this.getSlotAtPosition(var1, var2); ItemStack var7 =
		 * this.mc.player.inventory.getItemStack(); if(this.field_147007_t && var6 !=
		 * null && var7 != null && var6 instanceof SlotFalseCopy) { if(this.lastIndex !=
		 * var6.slotNumber) { this.lastIndex = var6.slotNumber;
		 * this.handleMouseClick(var6, var6.slotNumber, 0, 0); } } else { }
		 */
		this.lastIndex = -1;
	...
	protected void AddMainPlayerSlots(Container container, GuiElementList elements, String type, Color color, int x,
			int y) {
		AddPlayerSlots(x, y, container, elements, true, false, type, color);
	}

	protected void AddHotbarPlayerSlots(Container container, GuiElementList elements, String type, Color color, int x,
			int y) {
		AddPlayerSlots(x, y, container, elements, false, true, type, color);
	}

	protected void AddPlayerSlots(int x, int y, Container container, GuiElementList elements, boolean main,
			boolean hotbar, String type, Color color) {
		for (int i = 0; i < container.inventorySlots.size(); i++) {
			if (container.inventorySlots.get(i) instanceof SlotPlayerInventory) {
				...
				if (main && !slot.isHotbar()) {
					...
					inventorySlot.setPosition(x + (18 * (slot.getSlotIndex() % 9)),
							y + (18 * (slot.getSlotIndex() / 9)));
	...
	public void drawSizedModalRect(int x1, int y1, int x2, int y2, int color) {
		...
		BufferBuilder vb = var11.getBuffer();
		GL11.glEnable(3042);
		GL11.glDisable(3553);
	...
	public void drawSizedRect(int x1, int y1, int x2, int y2, int color) {
		...
		float var10 = (float) (color & 255) / 255.0F;
		Tessellator var11 = Tessellator.getInstance();
		BufferBuilder worldRenderer = var11.getBuffer();
		GL11.glDisable(3553);
		GlStateManager.color(var8, var9, var10, var7);
	...
	public void drawSizedTexturedModalRect(int x, int y, int u, int v, int width, int height, float texW, float texH) {
		...
		Tessellator var11 = Tessellator.getInstance();
		BufferBuilder wr = var11.getBuffer();
		wr.begin(7, DefaultVertexFormats.POSITION_TEX);
		wr.pos((double) (x), (double) (y + height), (double) this.zLevel)
				.tex((double) ((float) (u) * var9), (double) ((float) (v + height) * var10)).endVertex();
		wr.pos((double) (x + width), (double) (y + height), (double) this.zLevel)
				.tex((double) ((float) (u + width) * var9), (double) ((float) (v + height) * var10)).endVertex();
		wr.pos((double) (x + width), (double) (y), (double) this.zLevel)
				.tex((double) ((float) (u + width) * var9), (double) ((float) (v) * var10)).endVertex();
		wr.pos((double) (x), (double) (y), (double) this.zLevel)
				.tex((double) ((float) (u) * var9), (double) ((float) (v) * var10)).endVertex();
		var11.draw();
	}
	...
	public MOElementIconButton AddPage(ElementBaseGroup page, HoloIcon icon, String tooltip) {
		pages.add(page);
		MOElementIconButton button = new MOElementIconButton(this, this, 0, 0, page.getName(), 0, 0, 24, 0, 24, 0, 24,
				24, "", icon);
