...
public class ElementMatterStored extends MOElementBase {
	...
	// If this is enabled, 1 pixel of energy will always show in the bar as long as
	// it is non-zero.
	...
	protected int getScaled() {
		...
		return alwaysShowMinimum && storage.getMatterStored() > 0 ? Math.max(1, Math.round(fraction))
				: Math.round(fraction);
	}
