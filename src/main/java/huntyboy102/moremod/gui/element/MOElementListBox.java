		...
		while (nextElement < e && heightDrawn <= getContentHeight()) {
			if (shouldBeDisplayed(getElement(nextElement))) {
				...
				if (this._selectedIndex == nextElement) {
					DrawElement(nextElement, getContentLeft(), getContentTop() + heightDrawn, selectedLineColor,
							selectedTextColor, true, true);
				} else {
					DrawElement(nextElement, getContentLeft(), getContentTop() + heightDrawn, selectedLineColor,
							this.textColor, false, true);
				}
	...
	public void drawForeground(int mouseX, int mouseY) {
		...
		while (nextElement < e && heightDrawn <= getContentHeight()) {
			if (shouldBeDisplayed(getElement(nextElement))) {
				...
				if (getContentTop() + heightDrawn <= mouseY
						&& getContentTop() + heightDrawn + getElementHeight(nextElement) >= mouseY
