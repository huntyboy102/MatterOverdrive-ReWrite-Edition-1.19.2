...
import huntyboy102.moremod.gui.element.ElementCheckbox;
import huntyboy102.moremod.gui.element.ElementIntegerField;
import huntyboy102.moremod.gui.element.ElementStates;
	...
	public void init() {
		...
		for (IConfigProperty configProperty : componentConfigs.getValues().values()) {
			if (configProperty instanceof ConfigPropertyBoolean) {
				ElementCheckbox elementCheckbox = new ElementCheckbox(gui, this, 0, yPos, configProperty.getKey(),
						(boolean) configProperty.getValue());
				...
				if (configProperty instanceof ConfigPropertyStringList) {
					ElementStates elementStates = new ElementStates(gui, this, 0, yPos, configProperty.getKey(),
							((ConfigPropertyStringList) configProperty).getList());
					...
					ElementIntegerField elementIntegerField = new ElementIntegerField(gui, this, 0, yPos, 20,
							((ConfigPropertyInteger) configProperty).getMin(),
							((ConfigPropertyInteger) configProperty).getMax());
