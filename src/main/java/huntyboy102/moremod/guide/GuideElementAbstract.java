...
public abstract class GuideElementAbstract implements IGuideElement {
	...
	protected void loadStyles(MOGuideEntry entry, Element element, Map<String, String> styleMap, int width,
			int height) {
	...
	protected void calculateDimentions(MOGuideEntry entry, Element element, Map<String, String> styleMap,
	...
	private int parseStyleNumber(String number, int value) {
		if (number.endsWith("px")) {
			try {
				...
				MOLog.log(Level.ERROR, e, "Could not parse Int %s in %s", number.substring(0, number.length() - 2),
						this);
			}
			...
			try {
				...
				MOLog.log(Level.ERROR, e, "Could not parse Int %s in %s", number.substring(0, number.length() - 1),
						this);
			}
		}
		return 0;
