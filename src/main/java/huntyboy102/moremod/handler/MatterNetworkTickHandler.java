...
import huntyboy102.moremod.api.network.IMatterNetworkHandler;
...
public class MatterNetworkTickHandler implements IConfigSubscriber {
	...
	public void onWorldTickPost(TickEvent.Phase phase, World world) {
		if (broadcastCount >= max_broadcasts) {
			// if the broadcast count exceeded the maximum then store the last ID from the
			// ID count.
			// this will restart the broadcasting next tick from the last broadcaster not
			// the beginning.
			last_ID = id_count;
		} else {
	...
	public void onConfigChanged(ConfigurationHandler config) {
		this.max_broadcasts = config.getInt(ConfigurationHandler.KEY_MAX_BROADCASTS,
				ConfigurationHandler.CATEGORY_MATTER_NETWORK, 128,
