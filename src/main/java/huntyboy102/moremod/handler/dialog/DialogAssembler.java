...
package huntyboy102.moremod.handler.dialog;
...
import huntyboy102.moremod.data.dialog.DialogOptionReference;
...
public class DialogAssembler {
	private static final ResourceLocation dialogLocation = new ResourceLocation(
			Reference.MOD_ID + ":" + "dialog/dialog.json");
	...
	public IDialogOption parseOption(JsonElement element, DialogRegistry dialogRegistry) {
		if (element.isJsonObject()) {
			...
			if (element.getAsJsonObject().has("id")) {
				dialogRegistry.registerMessage(
						new ResourceLocation(MOJsonHelper.getString(element.getAsJsonObject(), "id")), optionMessage);
			} else {
