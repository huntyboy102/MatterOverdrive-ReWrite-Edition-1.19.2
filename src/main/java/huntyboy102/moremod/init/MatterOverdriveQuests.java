...
public class MatterOverdriveQuests {
	...
	private static void initMatterOverdriveQuests() {
		...
		trade_route = new GenericMultiQuest(new IQuestLogic[] { new QuestLogicBlockInteract(null, true, false),
				...
				"trade_route", 180).setAutoComplete(true).setSequential(true);
	}

	private static void initModdedQuests() {
		/*
		 * toThePowerOf = (GenericQuest) new GenericQuest(new QuestLogicCraft(new
		 * QuestItem[]{ new QuestItem("BigReactors:BRReactorPart", "BigReactors"), new
		 * QuestItem("ExtraUtilities:generator", "ExtraUtilities", 1, 1), new
		 * QuestItem(new ItemStack(MatterOverdrive.ITEMS.battery)) }, 1, 1,
		 * 120).setRandomItem(false).setAutoComplete(true), "to_the_power_of",
		 * 0).addQuestRewards(new ItemStackReward(MatterOverdrive.ITEMS.tritanium_ingot,
		 * 10), new ItemStackReward(MatterOverdrive.ITEMS.tritanium_plate, 4));
		 */
	}
