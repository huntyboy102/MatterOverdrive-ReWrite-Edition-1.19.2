...
public class NetworkFlashDrive extends MOBaseItem implements IMatterNetworkFilter {
	...
	public void addDetails(ItemStack itemstack, EntityPlayer player, @Nullable World worldIn, List<String> infos) {
		...
		if (itemstack.hasTagCompound()) {
			NBTTagList list = itemstack.getTagCompound().getTagList(IMatterNetworkFilter.CONNECTIONS_TAG,
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (tileEntity instanceof IMatterNetworkConnection) {
			...
			NBTTagList list = stack.getTagCompound().getTagList(IMatterNetworkFilter.CONNECTIONS_TAG,
					Constants.NBT.TAG_LONG);
			for (int i = 0; i < list.tagCount(); i++) {
