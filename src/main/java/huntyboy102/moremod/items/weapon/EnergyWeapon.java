...
import huntyboy102.moremod.init.MatterOverdriveEnchantments;
...
import huntyboy102.moremod.util.EntityDamageSourcePhaser;
import huntyboy102.moremod.util.MOEnergyHelper;
import huntyboy102.moremod.util.WeaponHelper;
import huntyboy102.moremod.util.animation.MOEasing;
...
public abstract class EnergyWeapon extends MOItemEnergyContainer implements IWeapon {
	...
	public void onUpdate(ItemStack itemStack, World world, Entity entity, int slot, boolean isHolding) {
		if (!world.isRemote) {
			...
			if (entity instanceof EntityPlayer && entity == Minecraft.getMinecraft().player
					&& ((EntityPlayer) entity).getHeldItemMainhand() == itemStack
	...
	public void addDetails(ItemStack weapon, EntityPlayer player, @Nullable World worldIn, List<String> infos) {
		...
		String energyInfo = TextFormatting.DARK_RED + "Power Use: "
				+ MOEnergyHelper.formatEnergy(null, getEnergyUse(weapon) * 20) + "/s";
		...
		infos.add(addStatWithMultiplyInfo("Speed", (int) (20d / getShootCooldown(weapon) * 60),
				(double) getBaseShootCooldown(weapon) / (double) getShootCooldown(weapon), " s/m"));
		infos.add(addStatWithMultiplyInfo("Range", getRange(weapon), (double) getRange(weapon) / (double) defaultRange,
				"b"));
		infos.add(addStatWithMultiplyInfo("Accuracy", "", 1 / (modifyStatFromModules(WeaponStats.ACCURACY, weapon, 1)
				* getCustomFloatStat(weapon, CUSTOM_ACCURACY_MULTIPLY_TAG, 1)), ""));
...
//		System.out.println("statName: "+ statName + " Multiply: " + multiply);
		...
		if (multiply != 1) {
			...
			info += String.format(" (%s) %s", DecimalFormat.getPercentInstance().format(multiply),
	...
	private void AddModuleDetails(ItemStack weapon, List infos) {
		...
		if (!module.isEmpty()) {
			 ...
			 * 
			 * Object statsObject = ((IWeaponModule)module.getItem()).getValue(module); if
			 * (statsObject instanceof Map) { for (final Map.Entry<Integer, Double> entry :
			 * ((Map<Integer,Double>) statsObject).entrySet()) { if (entry.getKey() !=
			 * Reference.WS_DAMAGE && entry.getKey() != Reference.WS_AMMO) {
			 * infos.add("    " + MOStringHelper.weaponStatToInfo(entry.getKey(),
			 * entry.getValue())); } } }
			 * 
			 * infos.add("");
			 */
	...
	/**
	 * Used to manage the sending of weapons tick to the server. This is tied to
	 * {@link #sendShootTickToServer(World, WeaponShot, Vec3d, Vec3d)}. Used to
	 * mainly send weapon ticks without weapon shot information, aka. weapon firing.
	 *
	...
	/**
	 * A utility function for sending a weapon tick with a given weapon shot to the
	 * server. This is part of the latency compensation system for Phaser bolts.
	 *
	 * @param world      the shooter's world.
	...
	/**
	 * This method is called each weapon update tick, when the player is holding the
	 * weapon and is not in any GUI screen. This is manly done for convenience.
	 *
	 * @param weapon         the weapon stack.
	 * @param world          the shooter world.
	 * @param entityPlayer   the player shooter.
	 * @param sendServerTick should a server weapon tick be sent. This is tied up to
	 *                       {@link #manageClientServerTicks(World)}.
	 */
	@SideOnly(Side.CLIENT)
	public void onShooterClientUpdate(ItemStack weapon, World world, EntityPlayer entityPlayer,
			...
			for (int i = 0; i < player.inventory.mainInventory.size(); i++) {
				...
				if (!stack.isEmpty() && stack.getItem() instanceof IEnergyPack && stack.getCount() > 0) {
					...
					if (container instanceof EnergyContainer)
						((EnergyContainer) container).setEnergy(
								container.getEnergyStored() + ((IEnergyPack) stack.getItem()).getEnergyAmount(stack));
					// player.inventory.inventoryChanged = true;
					player.world.playSound(null, player.getPosition(), MatterOverdriveSounds.weaponsReload,
							SoundCategory.PLAYERS, 0.7f + itemRand.nextFloat() * 0.2f,
	...
	protected void manageOverheat(ItemStack itemStack, World world, EntityLivingBase shooter) {
		if (getHeat(itemStack) >= getMaxHeat(itemStack)) {
			if (!MinecraftForge.EVENT_BUS.post(new MOEventEnergyWeapon.Overheat(itemStack, shooter))) {
				...
				world.playSound(null, shooter.posX, shooter.posY, shooter.posZ, MatterOverdriveSounds.weaponsOverheat,
						SoundCategory.PLAYERS, 1F, 1f);
				world.playSound(null, shooter.posX, shooter.posY, shooter.posZ,
						MatterOverdriveSounds.weaponsOverheatAlarm, SoundCategory.PLAYERS, 1, 1);
			}
		}
	}
	...
	/**
	 * Returns the speed on the Phaser Blot or any other projectile shot by the
	 * weapon. This is different than the {@link #getBaseShootCooldown(ItemStack)}.
	 *
	...
	/**
	 * Called when a weapon is fired with a single Weapon Shot. This is different
	 * then the
	 * {@link #onServerFire(ItemStack, EntityLivingBase, WeaponShot, Vec3d, Vec3d, int)}
	 * in that it's called only on the client side.
	 *
	 * @param weapon   the weapon.
	 * @param shooter  the shooter.
	 ...
	 */
	@SideOnly(Side.CLIENT)
	public abstract void onClientShot(ItemStack weapon, EntityLivingBase shooter, Vec3d position, Vec3d dir,
			WeaponShot shot);

	/**
	 * Called when a Phaser Bolt or another projectile has hit something. This
	 * method is only called on the client side.
	 *
	 * @param hit    the projective hit information.
	 * @param weapon the weapon.
	 * @param world  the world.
	 * @param amount the amount of "hit" there was by the projectile. This is mainly
	 *               used to control the amount of particles spawned on hit.
	 */
	@SideOnly(Side.CLIENT)
	...
	public PlasmaBolt spawnProjectile(ItemStack weapon, EntityLivingBase shooter, Vec3d position, Vec3d dir,
			WeaponShot shot) {
	...
	public WeaponShot createClientShot(ItemStack weapon, EntityLivingBase shooter, boolean zoomed) {
		return ((ClientWeaponHandler) MatterOverdrive.PROXY.getWeaponHandler()).getNextShot(weapon, this, shooter,
				zoomed);
	}

	public WeaponShot createServerShot(ItemStack weaponStack, EntityLivingBase shooter, boolean zoomed) {
		return new WeaponShot(itemRand.nextInt(), getWeaponScaledDamage(weaponStack, shooter),
				getAccuracy(weaponStack, shooter, zoomed), WeaponHelper.getColor(weaponStack), getRange(weaponStack));
	}
	...
	public float getWeaponScaledDamage(ItemStack weapon, EntityLivingBase shooter) {
		...
		damage += shooter.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.ATTACK_DAMAGE)
				.getAttributeValue();
		return damage;
	}
	...
	public int getEnergyUse(ItemStack weapon) {
		...
		energyUse -= energyUse
				* EnchantmentHelper.getEnchantmentLevel(Enchantment.getEnchantmentByLocation("unbreaking"), weapon)
				* 0.04f;
