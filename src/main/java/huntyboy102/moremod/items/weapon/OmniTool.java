...
package huntyboy102.moremod.items.weapon;/* Created by Simeon on 10/17/2015. */
...
import huntyboy102.moremod.fx.PhaserBoltRecoil;
...
public class OmniTool extends EnergyWeapon {
		...
		if (player.world.isRemote && player.equals(Minecraft.getMinecraft().player)) {
			if (canDig(stack, player.world)) {
				...
				if (hit != null && hit.typeOfHit == RayTraceResult.Type.BLOCK) {
					...
					final float hardness = state.getPlayerRelativeBlockHardness((EntityPlayer) player, player.world,
							hit.getBlockPos());
					boolean canMine = state.getBlock().canHarvestBlock(player.world, hit.getBlockPos(),
							(EntityPlayer) player);
					if (!state.getBlock().isAir(state, player.world, hit.getBlockPos()) && canMine) {
						...
						if (isSameBlock(hit.getBlockPos())) {
							if (BLOCK_DAMAGE >= 1.0F) {
								MatterOverdrive.NETWORK.sendToServer(new PacketDigBlock(hit.getBlockPos(),
										PacketDigBlock.Type.HARVEST, hit.sideHit));
								...
								MatterOverdrive.NETWORK.sendToServer(
										new PacketDigBlock(hit.getBlockPos(), PacketDigBlock.Type.CLICK, hit.sideHit));
							}
							BLOCK_DAMAGE = MathHelper.clamp(modifyStatFromModules(WeaponStats.BLOCK_DAMAGE, stack,
									(BLOCK_DAMAGE + hardness) * 2), 0, 1);
							player.world.sendBlockBreakProgress(player.getEntityId(), hit.getBlockPos(),
									(int) (BLOCK_DAMAGE * 10));
						} else {
	...
	private void stopMiningLastBlock(EntityPlayer player, World world) {
		if (CURRENT_BLOCK != null) {
			...
			MatterOverdrive.NETWORK
					.sendToServer(new PacketDigBlock(CURRENT_BLOCK, PacketDigBlock.Type.CANCEL, LAST_SIDE));
			world.sendBlockBreakProgress(player.getEntityId(), CURRENT_BLOCK, -1);
		}
	}

	@Override
	public boolean onBlockDestroyed(ItemStack p_onBlockDestroyed_1_, World p_onBlockDestroyed_2_,
			IBlockState p_onBlockDestroyed_3_, BlockPos p_onBlockDestroyed_4_, EntityLivingBase p_onBlockDestroyed_5_) {
		return super.onBlockDestroyed(p_onBlockDestroyed_1_, p_onBlockDestroyed_2_, p_onBlockDestroyed_3_,
				p_onBlockDestroyed_4_, p_onBlockDestroyed_5_);
	}
	...
	public void onClientShot(ItemStack weapon, EntityLivingBase shooter, Vec3d position, Vec3d dir, WeaponShot shot) {
		...
		MOPositionedSound sound = new MOPositionedSound(MatterOverdriveSounds.weaponsLaserFire, SoundCategory.PLAYERS,
				0.5f + itemRand.nextFloat() * 0.2f, 1.2f + itemRand.nextFloat() * 0.2f);
	...
	public boolean supportsModule(ItemStack weapon, ItemStack module) {
		return !StackUtils.isNullOrEmpty(module) && !StackUtils.isNullOrEmpty(weapon)
				&& (module.getItem() == MatterOverdrive.ITEMS.weapon_module_color
						|| (module.getItem() == MatterOverdrive.ITEMS.weapon_module_barrel
								&& module.getItemDamage() == WeaponModuleBarrel.BLOCK_BARREL_ID  && module.getItemDamage() != WeaponModuleBarrel.EXPLOSION_BARREL_ID));
	}
