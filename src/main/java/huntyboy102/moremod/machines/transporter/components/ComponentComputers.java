...
package huntyboy102.moremod.machines.transporter.components;
...
public class ComponentComputers extends MachineComponentAbstract<TileEntityMachineTransporter>// implements IPeripheral
{
	 ...
	 * "getSelectedLocation", "getLocation", "addLocation", "setSelectedLocation",
	 * "setName", "setX", "setY", "setZ", "setRedstoneMode" }; private String
	 * peripheralName = "mo_transporter";
	 * 
	 * public ComponentComputers(TileEntityMachineTransporter machine) {
	 * super(machine); }
	 * 
	 * 
	 * private Object[] callMethod(int method, Object[] args) { switch (method) {
	 * case 0: return computerGetLocations(args); case 1: return
	 * computerGetSelectedLocation(args); case 2: return computerGetLocation(args);
	 * case 3: return computerAddLocation(args); case 4: return
	 * computerSetSelectedLocation(args); case 5: return computerSetName(args); case
	 * 6: return computerSetX(args); case 7: return computerSetY(args); case 8:
	 * return computerSetZ(args); case 9: return computerSetRedstoneMode(args);
	 * default: throw new IllegalArgumentException("Invalid method id"); } }
	 * 
	 * private Object[] computerGetLocations(Object[] args) {
	 * 
	 * ArrayList<HashMap<String, Object>> list = new ArrayList<>(); for
	 * (TransportLocation loc : machine.getPositions()) { HashMap<String, Object>
	 * map = new HashMap<>();
	 * 
	 * map.put("name", loc.name); map.put("selected", machine.selectedLocation ==
	 * machine.getPositions().indexOf(loc)); map.put("x", loc.pos.getX());
	 * map.put("y", loc.pos.getY()); map.put("z", loc.pos.getZ());
	 * 
	 * list.add(map); }
	 * 
	 * return list.toArray(); }
	 * 
	 * private Object[] computerGetSelectedLocation(Object[] args) { return
	 * computerGetLocation(new Object[]{1.0}); }
	 * 
	 *//**
	 ...
	 * 
	 * if (!(args[0] instanceof Double)) { throw new
	 * IllegalArgumentException("First argument must be the numerical id of the transport location"
	 * ); }
	 * 
	 ...
	 * 
	 * TransportLocation loc = machine.getPositions().get(locNum); map.put("name",
	 * loc.name); map.put("x", loc.pos.getX()); map.put("y", loc.pos.getY());
	 ...
	 *//**
		 ...
		 */
	/*
	 * private Object[] computerAddLocation(Object[] args) { if (!(args[0]
	 * instanceof String)) { throw new
	 * IllegalArgumentException("First argument must be a string containing the name of the transport location"
	 * ); } for (int i = 1; i <= 4; i++) { if (!(args[i] instanceof Double)) { throw
	 * new IllegalArgumentException("Argument " + i + 1 + " must be an integer"); }
	 * } String name = (String)args[0]; int x = (int)Math.floor((Double)args[1]);
	 * int y = (int)Math.floor((Double)args[2]); int z =
	 * (int)Math.floor((Double)args[3]); machine.addNewLocation(new BlockPos(x, y,
	 * z), name); return null; }
	 * 
	 *//**
		 * args: id (number) numeric index of the location to select (First location has
		 ...
		 */
	/*
	 * private Object[] computerSetSelectedLocation(Object[] args) { if (!(args[0]
	 * instanceof Double)) { throw new
	 * IllegalArgumentException("Argument 1 must be a number"); }
	 * machine.selectedLocation = (int)Math.floor((Double)args[0]); return null; }
	 * 
	 *//**
		 * args: id (number) numeric index of the location to select (First location has
		 ...
		 */
	/*
	 * private Object[] computerSetName(Object[] args) { if (!(args[0] instanceof
	 * Double)) { throw new IllegalArgumentException("Argument 1 must be a number");
	 * } if (!(args[1] instanceof String)) { throw new
	 * IllegalArgumentException("Argument 2 must be a string"); }
	 * 
	 * int locNum = (int)Math.floor((Double)args[0]);
	 * 
	 ...
	 *//**
		 ...
		 */
	/*
	 * private Object[] computerSetX(Object[] args) { if (!(args[0] instanceof
	 * Double)) { throw new IllegalArgumentException("Argument 1 must be a number");
	 * } if (!(args[1] instanceof Double)) { throw new
	 * IllegalArgumentException("Argument 2 must be a number"); }
	 * 
	 * int locNum = (int)Math.floor((Double)args[0]);
	 ...
	 *//**
		 ...
		 */
	/*
	 * private Object[] computerSetY(Object[] args) { if (!(args[0] instanceof
	 * Double)) { throw new IllegalArgumentException("Argument 1 must be a number");
	 * } if (!(args[1] instanceof Double)) { throw new
	 * IllegalArgumentException("Argument 2 must be a number"); }
	 * 
	 ...
	 *//**
		 ...
		 *//*
			 * private Object[] computerSetZ(Object[] args) { if (!(args[0] instanceof
			 * Double)) { throw new IllegalArgumentException("Argument 1 must be a number");
			 * } if (!(args[1] instanceof Double)) { throw new
			 * IllegalArgumentException("Argument 2 must be a number"); }
			 * 
	 ...
	 *//*
		 * private Object[] computerSetRedstoneMode(Object[] args) { if (!(args[0]
		 * instanceof Double)) { throw new
		 * IllegalArgumentException("Argument 1 must be a number from 0 to 2"); }
		 * 
		 * int i = (int)Math.floor((Double)args[0]);
		 * 
		 * if (i < 0 || i > 2) { throw new
		 * IllegalArgumentException("Argument 1 must be a number from 0 to 2"); }
		 * 
		 * IConfigProperty property =
		 * machine.getConfigs().getProperty(Reference.CONFIG_KEY_REDSTONE_MODE); if
		 * (property != null) { property.setValue(i); }else { throw new
		 * IllegalArgumentException("No redstone mode config found for machine"); }
		 * 
		 * return null; }
		 * 
		 * 
