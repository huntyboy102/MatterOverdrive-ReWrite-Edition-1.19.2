...
import huntyboy102.moremod.fx.AndroidTeleportParticle;
...
public class PacketSpawnParticle extends PacketAbstract {
	...
	public PacketSpawnParticle(String particleType, double x, double y, double z, int count,
			RenderParticlesHandler.Blending blending) {
		this(particleType, x, y, z, count, blending, 0);
	}

	public PacketSpawnParticle(String particleType, double x, double y, double z, int count,
			RenderParticlesHandler.Blending blending, float size) {
		this(particleType, new double[] { x, y, z }, count, blending, size);
	}

	public PacketSpawnParticle(String particleType, double[] inates, int count,
			RenderParticlesHandler.Blending blending, float size) {
		...
		public void spawnParticle(World world, PacketSpawnParticle message) {
			...
			if (message.particleType.equalsIgnoreCase("teleport")) {
				...
				if (message.inates.length > 7) {
					particle = new Lightning(world, new Vec3d(message.inates[0], message.inates[1], message.inates[2]),
							new Vec3d(message.inates[3], message.inates[4], message.inates[5]),
							(float) message.inates[6], (float) message.inates[7]);
