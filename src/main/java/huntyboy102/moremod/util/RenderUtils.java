...
public class RenderUtils {
	...
	public static void renderStack(int x, int y, int z, ItemStack stack, boolean renderOverlay) {
		if (!stack.isEmpty()) {
			...
			if (renderOverlay) {
				renderItemOverlayIntoGUI(font, stack, x, y,
						stack.getCount() > 1 ? Integer.toString(stack.getCount()) : null);
			}
	...
	public static void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition,
		...
		if (!stack.isEmpty()) {
			...
			if (stack.getItem().showDurabilityBar(stack)) {
				...
				// GlStateManager.enableBlend(); // Forge: Disable Blend because it screws with
				// a lot of things down the line.
				GlStateManager.enableAlpha();
...
//TODO
	...
	public static void drawPlaneWithUV(double sizeX, double sizeY, double uStart, double vStart, double uSize,
			double vSize) {
		drawPlaneWithUV(0, 0, 0, sizeX, sizeY, uStart, vStart, uSize, vSize);
	}

	public static void drawPlaneWithUV(double x, double y, double z, double sizeX, double sizeY, double uStart,
			double vStart, double uSize, double vSize) {
	...
	public static void drawPlaneWithUV(double x, double y, double z, double sizeX, double sizeY, double uStart,
			double vStart, double uSize, double vSize, Color color) {
		BufferBuilder wr = Tessellator.getInstance().getBuffer();

		wr.begin(GL_QUADS, DefaultVertexFormats.POSITION_TEX_COLOR);
		wr.pos(x + sizeX, y + sizeY, z).tex(uStart + uSize, vStart + vSize)
				.color(color.getFloatR(), color.getFloatG(), color.getFloatB(), color.getFloatA()).endVertex();
		wr.pos(x + sizeX, y, z).tex(uStart + uSize, vStart)
				.color(color.getFloatR(), color.getFloatG(), color.getFloatB(), color.getFloatA()).endVertex();
		wr.pos(x, y, z).tex(uStart, vStart)
				.color(color.getFloatR(), color.getFloatG(), color.getFloatB(), color.getFloatA()).endVertex();
		wr.pos(x, y + sizeY, z).tex(uStart, vStart + vSize)
				.color(color.getFloatR(), color.getFloatG(), color.getFloatB(), color.getFloatA()).endVertex();
		Tessellator.getInstance().draw();
	}
	...
	public static void drawCube(double sizeX, double sizeY, double sizeZ, Color color, float multiply) {
		drawCube(0, 0, 0, sizeX, sizeY, sizeZ, new Color((int) (color.getIntR() * multiply),
				(int) (color.getIntG() * multiply), (int) (color.getIntB() * multiply)));
	}
	...
	public static void drawCube(double x, double y, double z, double sizeX, double sizeY, double sizeZ, double minU,
			double minV, double maxU, double maxV, Color color) {
		BufferBuilder wr = Tessellator.getInstance().getBuffer();
	...
	/*
	 * public static void tesseleteModelAsBlock(Matrix4f mat,GroupObject
	 * object,IIcon icon,int x,int y,int z,int brightness,boolean lighting,Color
	 * color) { float uSize = icon.getMaxU() - icon.getMinU(); float vSize =
	 * icon.getMaxV() - icon.getMinV(); float textureOffset = 0.00005F; Vector4f pos
	 * = new Vector4f(0,0,0,1);
	 * 
	 * for (Face face : object.faces) { if (face.faceNormal == null) {
	 * face.faceNormal = face.calculateFaceNormal(); } float averageU = 0F; float
	 * averageV = 0F;
	 * 
	 * if ((face.textureinates != null) && (face.textureinates.length > 0)) { for
	 * (int i = 0; i < face.textureinates.length; ++i) { averageU +=
	 * face.textureinates[i].u; averageV += face.textureinates[i].v; }
	 * 
	 * averageU = averageU / face.textureinates.length; averageV = averageV /
	 * face.textureinates.length; }
	 * 
	 * float offsetU, offsetV;
	 * 
	 * for (int i = 0; i < face.vertices.length; ++i) {
	 * 
	 * if ((face.textureinates != null) && (face.textureinates.length > 0)) {
	 * offsetU = textureOffset; offsetV = textureOffset;
	 * 
	 * if (face.textureinates[i].u > averageU) { offsetU = -offsetU; } if
	 * (face.textureinates[i].v > averageV) { offsetV = -offsetV; }
	 * 
	 * pos.x = face.vertices[i].x; pos.y = face.vertices[i].y; pos.z =
	 * face.vertices[i].z; pos.w = 1; Matrix4f.transform(mat, pos, pos);
	 * 
	 * Tessellator.instance.setNormal(face.faceNormal.x, face.faceNormal.y,
	 * face.faceNormal.z); float colorMul = 1f; if (lighting) { colorMul = 0.7f +
	 * 0.3f * Vector3f.dot(new
	 * Vector3f(face.faceNormal.x,face.faceNormal.y,face.faceNormal.z),new
	 * Vector3f(-0.3f,1,0)); } if (color != null) {
	 * Tessellator.instance.setColorOpaque_F(color.getFloatR() * colorMul,
	 * color.getFloatG() * colorMul, color.getFloatB() * colorMul); }else {
	 * Tessellator.instance.setColorOpaque_F(colorMul, colorMul, colorMul); } if
	 * (brightness >= 0) { Tessellator.instance.setBrightness(brightness); }
	 * Tessellator.instance.addVertexWithUV(x + 0.5f + pos.x, y + pos.y, z + 0.5f +
	 * pos.z, icon.getMinU() + face.textureinates[i].u * uSize + offsetU,
	 * icon.getMinV() + face.textureinates[i].v * vSize + offsetV); } else {
	 * Tessellator.instance.addVertex(x + face.vertices[i].x, y +
	 * face.vertices[i].y, z + face.vertices[i].z); } } } }
	 */
	...
	public static Color lerp(Color a, Color b, float lerp) {
		return new Color(lerp(a.getIntR(), b.getIntR(), lerp), lerp(a.getIntG(), b.getIntG(), lerp),
				lerp(a.getIntB(), b.getIntB(), lerp), lerp(a.getIntA(), b.getIntA(), lerp));
	}

	public static void applyColor(int color) {
		GlStateManager.color((float) (color >> 16 & 255) / 255f, (float) (color >> 8 & 255) / 255f,
				(float) (color & 255) / 256f, (float) (color >> 24 & 255) / 255f);
	}
	...
	public static void applyColorWithMultipy(int color, float mul) {
		GlStateManager.color((color >> 16 & 255) / 255f * mul, (color >> 8 & 255) / 255f * mul,
				(color & 255) / 256f * mul, (float) (color >> 24 & 255) / 255f);
	}
	...
	public static void beginDrawinngBlockScreen(double x, double y, double z, Direction side, Color color,
		...
		Vector3f dir = new Vector3f(side.getDirectionVec().getX(), side.getDirectionVec().getY(),
		...
		float multiply = (float) (MOMathHelper.noise(entity.getPos().getX(), entity.getWorld().getWorldTime() * 0.01,
				entity.getPos().getZ()) * 0.5 + 0.5) * glowAlpha;
	...
	public static void DrawMultilineInfoCentered(List<?> infos, int x, int y, int maxLines, int maxLineWidth,
			int color) {
		try {
	...
	public static void drawSizedTexturedModalRect(int x, int y, int u, int v, int width, int height, int widthU,
			int heightV, float texW, float texH, float zLevel) {
	...
	public static void tessalateSizedModelRect(int x, int y, int u, int v, int width, int height, int widthU,
			int heightV, float texW, float texH, float zLevel) {
		float texU = 1 / texW;
		float texV = 1 / texH;
		BufferBuilder wr = Tessellator.getInstance().getBuffer();
