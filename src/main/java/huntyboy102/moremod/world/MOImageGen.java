...
public abstract class MOImageGen<T extends MOImageGen.ImageGenWorker> {
	...
	public void generateFromImage(World world, Random random, BlockPos start, int layer, int placeNotify, T worker) {
		if (layers != null && layers.size() > 0) {
			...
			generateFromImage(world, random,
					new BlockPos(start.getX(), Math.min(start.getY(), world.getHeight() - layerCount), start.getZ()),
					layers, layer, placeNotify, worker);
		}
	}

	public void generateFromImage(World world, Random random, BlockPos start, List<int[][]> layers, int layer,
			int placeNotify, T worker) {
	...
	public static void generateFromImage(World world, BlockPos start, int layerWidth, int layerHeight,
			List<int[][]> layers, Map<Integer, IBlockState> blockMap) {
	...
	public boolean isOnSolidGround(World world, BlockPos pos, int leaway) {
		return isPointOnSolidGround(world, pos, leaway)
				&& isPointOnSolidGround(world, pos.add(layerWidth, 0, 0), leaway)
				&& isPointOnSolidGround(world, pos.add(layerWidth, 0, layerHeight), leaway)
				&& isPointOnSolidGround(world, pos.add(0, 0, layerHeight), leaway);
	}
	...
	public boolean canFit(World world, BlockPos pos) {
		return !isBlockSolid(world, pos.add(0, layerCount, 0))
				&& !isBlockSolid(world, pos.add(layerCount, layerCount, 0))
				&& !isBlockSolid(world, pos.add(layerCount, layerCount, layerCount))
				&& !isBlockSolid(world, pos.add(0, layerCount, layerCount));
	}

	public boolean isBlockSolid(World world, BlockPos pos) {
		IBlockState block = world.getBlockState(pos);
		if (block.getBlock() == Blocks.LOG || block.getBlock() == Blocks.LOG2 && block.getBlock() == Blocks.LEAVES2
				|| block.getBlock() == Blocks.LEAVES) {
			return false;
		}
	...
	private void loadTexture(ResourceLocation textureLocation) throws RuntimeException {
		try {
			...
			String path = "/assets/matteroverdrive/" + textureLocation.getPath();
	...
	private static void convertTo2DWithoutUsingGetRGB(BufferedImage image, int layerWidth, int layerHeight,
			int textureWidth, List<int[][]> layers) {
		...
		if (hasAlphaChannel) {
			...
			for (int pixel = 0, row = 0, col = 0; pixel < pixels.length; pixel += pixelLength) {
				...
				int layerIndex = Math.floorDiv(col, layerWidth)
						+ ((textureWidth / layerWidth) * (Math.floorDiv(row, layerHeight)));
	...
	public abstract static class ImageGenWorker {
		...
		public void init(Random random, BlockPos pos, World world, IChunkGenerator chunkGenerator,
				IChunkProvider chunkProvider) {
			this.pos = pos;
			this.random = random;
			this.world = world;
	...
	public T createWorker(Random random, BlockPos pos, World world, IChunkGenerator chunkGenerator,
			IChunkProvider chunkProvider) {
