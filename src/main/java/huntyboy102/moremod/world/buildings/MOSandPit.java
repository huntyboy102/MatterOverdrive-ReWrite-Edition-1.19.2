...
public class MOSandPit extends MOWorldGenBuilding {
	...
	public boolean isFlat(World world, BlockPos pos) {
		...
		if (Math.abs(pos.getY() - y10.getY()) <= airLeeway && Math.abs(pos.getY() - y11.getY()) <= airLeeway
			...
			return blockBelowMatches(airLeeway, world, Blocks.SAND, pos)
					&& blockBelowMatches(airLeeway, world, Blocks.SAND, pos.add(layerWidth, 0, 0))
					&& blockBelowMatches(airLeeway, world, Blocks.SAND, pos.add(0, 0, layerHeight))
					&& blockBelowMatches(airLeeway, world, Blocks.SAND, pos.add(layerWidth, 0, layerHeight));
		}
		return false;
	}
	...
	@Override
	public void placeBlock(World world, int color, BlockPos pos, int layer, Random random, int placeNotify,
